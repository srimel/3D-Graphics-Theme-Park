...
bool cube::Initialize(float pos_x, float pos_y, float pos_z, float scale_x, float scale_y, float scale_z, const char * fn)
{
	x = pos_x; y = pos_y; z = pos_z;
    ...
    // Load the image for the texture. The texture file has to be in
    // a place where it will be found.
    if ( ! ( image_data = (ubyte*)tga_load(fn, &image_width,
					   &image_height, TGA_TRUECOLOR_24) ) )
    {
	fprintf(stderr, "cube::Initialize: Couldn't load tga file\n");
	return false;
    }
	...
	glNormal3f(0.0, 0.0, 1.0);
		glTexCoord2f(as+offset, at);
		glVertex3f((-1*scale_x)+x, (1*scale_y)+y, (1*scale_z)+z);
	    glTexCoord2f(bs+offset, bt);
		glVertex3f((-1*scale_x)+x, (-1*scale_y)+y, (1*scale_z)+z);
	    glTexCoord2f(cs, ct);
		glVertex3f((1*scale_x)+x, (-1*scale_y)+y, (1*scale_z)+z);
